...
```powershell
package.cmd
# 产物：dist\PDFBoard-1.11.2-setup.exe   （自带 Qt 运行库，约 17 MB）
```
...
```bat
PDFBoard-1.11.2-setup.exe /VERYSILENT /SUPPRESSMSGBOXES /NORESTART
rem 不要桌面快捷方式：
PDFBoard-1.11.2-setup.exe /VERYSILENT /SUPPRESSMSGBOXES /NORESTART /TASKS=""
```
...
`PDFBoard-nightly-setup.exe` / `PDFBoard-nightly-portable.zip`，版本号形如 `1.11.2-nightly.<提交号>`。
...
# 2. 提交推送
git commit -am "chore: 版本号 1.11.2"
git push
...
git tag -a v1.11.2 -F notes.txt
git push origin v1.11.2
```

> 不想用文件？也可以重复 `-m`，每次一行（Git 会把它们连成多段）：
> `git tag -a v1.11.2 -m "v1.11.2" -m "- 修复：…" -m "- 新增：…"`
>
> 忘记写注解（`git tag v1.11.2` 的轻量标签）不会失败，但 Release 只会是自动生成的提交列表——老师点开更新提示几乎看不到有用内容。
