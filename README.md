...
```powershell
package.cmd
# 产物：dist\PDFBoard-1.4.2-setup.exe   （自带 Qt 运行库，约 17 MB）
```
...
```bat
PDFBoard-1.4.2-setup.exe /VERYSILENT /SUPPRESSMSGBOXES /NORESTART
rem 不要桌面快捷方式：
PDFBoard-1.4.2-setup.exe /VERYSILENT /SUPPRESSMSGBOXES /NORESTART /TASKS=""
```
...
`PDFBoard-nightly-setup.exe` / `PDFBoard-nightly-portable.zip`，版本号形如 `1.4.2-nightly.<提交号>`。
