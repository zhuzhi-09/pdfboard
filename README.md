...
### 批注
- 手写笔：6 色 + 3 档粗细；若设备**上报橡皮尾**（`pointerType() == Eraser`），翻过来按即擦且不改动工具栏 ——
  注意：部分大屏**不区分笔尾与手指**，这类设备请用工具栏的「橡皮」按钮（见下方"已知限制"）
- **局部橡皮**：指哪擦哪（把笔迹在该处切开，两侧保留），不是整条删除
- **橡皮有三档粗细**：单击「橡皮」按钮可选 **大 / 中 / 小**（弹出的面板与笔的颜色面板同样式）
- **手掌当橡皮：实验性功能，默认关闭，多数面板上不可用**。多指触摸会被 **Windows 自身的
  手势识别**先接走，程序收不到完整的多指帧，因此"手掌 = 大接触面 = 自动橡皮"这条在系统层
  就被截断了（不是调参能解决的）。代码保留在「设置 → 通用 → 手掌当橡皮（实验性）」里，
  默认关闭；换成系统手势可控的面板时还有可能再捡起来
...
- **保存批注 → `.dpz` 批注包**（标准 ZIP 容器）：
  ...
  - 双击 `.dpz` 即可回到上次的批注状态
- **保存位置规则**（和 Word 一致）：打开 `.dpz` 时「保存」直接写回该文件；打开**普通 PDF /
  Word / 图片**时，第一次「保存」会弹出**另存为**让你挑位置 —— **永远不会**在你的原始文件旁边
  生成 `.dpz`，也不会把批注只留在临时文件里；关闭时若还有未保存的批注，同样会问你「保存 / 另存为 / 舍弃 / 取消」
- **另存为**：
  - `打包保存 (*.dpz)`（默认）
...
```powershell
package.cmd
# 产物：dist\PDFBoard-1.12.3-setup.exe   （自带 Qt 运行库，约 17 MB）
```
...
```bat
PDFBoard-1.12.3-setup.exe /VERYSILENT /SUPPRESSMSGBOXES /NORESTART
rem 不要桌面快捷方式：
PDFBoard-1.12.3-setup.exe /VERYSILENT /SUPPRESSMSGBOXES /NORESTART /TASKS=""
```
...
程序会在系统临时目录 `%TEMP%\pdfboard\` 保存**工作副本**（每个打开过的文档一份，用于快速恢复批注），
**每次启动会自动清理超过 14 天的**，所以长期使用也不会把磁盘占满（下次打开时再生成即可）。

诊断日志**默认关闭**（设置 → 调试）；即使打开，也只有「每秒一行」的运行探针和「每笔一行」的书写记录
（**不会按采样点逐点记录**），并且到 2 MB 自动轮转，不会无限增长。

### 打不开怎么办（Windows 上最常见的一种）

如果双击后提示 **“no Qt platform plugin could be initialized”**，或程序一闪而过，
通常是**那台电脑的依赖/安全软件**问题，而不是安装包坏了：

1. 安装一次 **Microsoft Visual C++ 2015-2022 Redistributable (x64)**，再打开程序；
2. 若装了火绒 / 360 等安全软件，把程序目录加入白名单（或临时退出后重试）；
3. 仍然不行时，把 **`%LOCALAPPDATA%\PDFBoard\logs\startup.log`** 发给我们：
   它从程序启动的第一行就开始记录 Qt 的消息、**本机的 Windows 版本（build 号）**、
   **随包运行库的版本**与**插件加载轨迹**（每次启动重写），能直接看出是缺哪个 DLL、
   还是搜索路径被别的软件改坏了。

> 安装器会在 **Windows 10 以下**拒绝安装（提示系统版本不符）。**Windows 10 各版本都能装**；
> Qt 官方只保证 1809 及以上，更老的 Win10 通常也能运行，遇到问题就把上面那个日志发我们。
...
`PDFBoard-nightly-setup.exe` / `PDFBoard-nightly-portable.zip`，版本号形如 `1.12.3-nightly.<提交号>`。
...
# 2. 提交推送
git commit -am "chore: 版本号 1.12.3"
git push
...
git tag -a v1.12.3 -F notes.txt
git push origin v1.12.3
```

> 不想用文件？也可以重复 `-m`，每次一行（Git 会把它们连成多段）：
> `git tag -a v1.12.3 -m "v1.12.3" -m "- 修复：…" -m "- 新增：…"`
>
> 忘记写注解（`git tag v1.12.3` 的轻量标签）不会失败，但 Release 只会是自动生成的提交列表——老师点开更新提示几乎看不到有用内容。
...
| 依赖 | 版本 |
|---|---|
| Windows | 10 **1709（内部版本 16299）及以上** / 11，64-bit |
...
- **低于 Windows 10 1709（内部版本 16299）的机器无法运行**：Qt 的平台插件需要
  `UIAutomationCore` 的 `UiaRaiseNotificationEvent`，该接口自 1709 起才提供（经实测确认）。
  这类机器启动时会看到**中文提示**（告知需要升级系统），而不是英文报错；诊断写入
  `%LOCALAPPDATA%\PDFBoard\logs\startup.log`。升级到 1709 及以上即可正常运行。
- **部分大屏不区分笔尾与手指**：硬件把笔的橡皮尾一律当手指上报（没有独立的工具类型），
  所以翻过来按不会自动变成橡皮，请用工具栏的「橡皮」按钮切换。若你的设备上报接触面积，
  未来可按"大接触面=橡皮"进一步自动化（见 `docs/03` 2.52）。
