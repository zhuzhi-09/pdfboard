...
- **橡皮有两档玩法**：① 单击「橡皮」按钮可**选大 / 中 / 小三档粗细**（弹出的面板与笔的颜色面板同样式）；
  ② **手掌按上去就自动变成橡皮**，且**擦除范围跟着接触面自动变大**（手掌压得越开、擦得越大）——
  此时不会缩放、不会平移。**一边掌托一边用另一根手指写字也照常书写，不会被误擦**。
  掌擦若在某块屏上误触发，可在「**设置 → 通用 → 手掌当橡皮**」一键关掉（**默认开**）
...
```powershell
package.cmd
# 产物：dist\PDFBoard-1.12.2-setup.exe   （自带 Qt 运行库，约 17 MB）
```
...
```bat
PDFBoard-1.12.2-setup.exe /VERYSILENT /SUPPRESSMSGBOXES /NORESTART
rem 不要桌面快捷方式：
PDFBoard-1.12.2-setup.exe /VERYSILENT /SUPPRESSMSGBOXES /NORESTART /TASKS=""
```
...
`PDFBoard-nightly-setup.exe` / `PDFBoard-nightly-portable.zip`，版本号形如 `1.12.2-nightly.<提交号>`。
...
# 2. 提交推送
git commit -am "chore: 版本号 1.12.2"
git push
...
git tag -a v1.12.2 -F notes.txt
git push origin v1.12.2
```

> 不想用文件？也可以重复 `-m`，每次一行（Git 会把它们连成多段）：
> `git tag -a v1.12.2 -m "v1.12.2" -m "- 修复：…" -m "- 新增：…"`
>
> 忘记写注解（`git tag v1.12.2` 的轻量标签）不会失败，但 Release 只会是自动生成的提交列表——老师点开更新提示几乎看不到有用内容。
