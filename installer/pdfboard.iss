...
;
; 静默安装（供教室集中管理客户端调用）�?;   PDFBoard-1.7.4-setup.exe /VERYSILENT /SUPPRESSMSGBOXES /NORESTART
;   PDFBoard-1.7.4-setup.exe /VERYSILENT /NORESTART /TASKS=""      ; 不建桌面快捷方式
; ---------------------------------------------------------------------------
...
[Setup]
; AppId 必须在所有版本间保持不变，否则升级会装成两份�?AppId={{9C1F4E2A-7B3D-4E58-9A21-6D5C0B8F3A71}
AppName={#AppName}
...
WizardStyle=modern
; 默认按当前用户安装（不弹 UAC，适合静默下发）；手动运行时可在向导里改为"为所有用�?�?PrivilegesRequired=lowest
...
; --- .dpz 批注包关�?---
Root: HKCU; Subkey: "Software\Classes\PDFBoard.Dpz"; ValueType: string; ValueName: ""; ValueData: "落墨批注�?; Flags: uninsdeletekey
...
Root: HKCU; Subkey: "Software\PDFBoard\Capabilities"; ValueType: string; ValueName: "ApplicationDescription"; ValueData: "大屏 PDF 查看与批注工�?
...
[UninstallDelete]
; 卸载时清掉从批注包里解出来的临时源文�?Type: filesandordirs; Name: "{tmp}\pdfboard"
